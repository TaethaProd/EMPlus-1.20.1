...
public class FactionBClassSelectScreen extends Screen {
	private final Identifier factionId;
	private final List<FactionClassCommandsConfig.ClassEntry> entries;

	public FactionBClassSelectScreen(Identifier factionId) {
		super(Text.literal("Классы фракции Б"));
		this.factionId = factionId;
		this.entries = prepareEntries(FactionClassCommandsConfigManager.get().factionB);
	}
	...
	protected void init() {
		...
		for (int i = 0; i < count; i++) {
			...
			String label = entry.label != null && !entry.label.isBlank() ? entry.label : "Класс B" + (i + 1);
			String classId = entry.id != null && !entry.id.isBlank() ? entry.id : "class_b" + (i + 1);
	...
	private List<FactionClassCommandsConfig.ClassEntry> prepareEntries(List<FactionClassCommandsConfig.ClassEntry> source) {
		...
		while (list.size() < 6) {
			...
			entry.id = "class_b" + (list.size() + 1);
			entry.label = "Класс B" + (list.size() + 1);
			list.add(entry);
		}
		if (list.size() > 6) {
